...
- ~~Add CI with automated build and test pipelines.~~ **Done, CI only** — GitHub Actions (`.github/workflows/ci.yml`) builds a real Postgres + HypoPG image, restores a versioned schema, regenerates the dataset, and runs the full 94-test suite on every push/PR to `main`/`consolidated`; frontend build runs alongside it. **The "CD" half is not done** — Render and Vercel still auto-deploy on push independently of this workflow's result; nothing gates a bad push from reaching the live URL. Deliberately deferred (see `CURRENT_PROGRESS.md`'s "Next, ranked"), not silently dropped.
